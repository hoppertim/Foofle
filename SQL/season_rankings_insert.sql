...
select sor.season_year
	...
	, round(cast( (sor.off_int - avg.int ) / std.int as numeric), 4) * -1.0 as int
	, round(cast( (sor.off_fumble - avg.fumble ) / std.fumble as numeric), 4) * -1.0 as fumble
	, round(cast( (sor.off_sack - avg.sack ) / std.sack as numeric), 4) * -1.0 as sack
...
select sdr.season_year
	, sdr.team_id
	, False
	, round(cast((sdr.def_pts - avg.pts) * 1.0 / std.pts as numeric), 4) * -1.0 as pts
	, round(cast((sdr.def_pass_yds - avg.pass_yds) * 1.0 / std.pass_yds as numeric), 4) * -1.0 as pass_yds
	, round(cast((sdr.def_pass_tds - avg.pass_tds) * 1.0 / std.pass_tds as numeric), 4) * -1.0 as pass_tds
	, round(cast((sdr.def_rush_yds - avg.rush_yds) * 1.0 / std.rush_yds as numeric), 4) * -1.0 as rush_yds
	, round(cast((sdr.def_rush_tds - avg.rush_tds) * 1.0 / std.rush_tds as numeric), 4) * -1.0 as rush_tds
